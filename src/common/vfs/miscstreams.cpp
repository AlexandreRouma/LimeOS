...
namespace miscstrms {
    ...
    void init() {
        fio::mountStream("/dev/null",  FS_FLAG_O_R | FS_FLAG_O_W, stream::create(0x1000, 0, _null_writeHndlr, _null_readHndlr, _closeHndlr, NULL));
        fio::mountStream("/dev/random",  FS_FLAG_O_R | FS_FLAG_O_W, stream::create(0x1000, 0, _random_writeHndlr, _random_readHndlr, _closeHndlr, NULL));
    }
}